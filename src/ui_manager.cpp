...
void GameplayUi::update_layout(int viewport_width, int viewport_height, const GameplayUiModel& model) {
    ...
    if (model.build_panel_open || model.farming_panel_open || model.active_tool == UiTool::decoration) {
        ...
        add_panel(panel_bounds); build_panel_bounds_ = panel_bounds;
        const float visible_height = panel_height - header_height - 8.0F;
        const std::vector<UiBuildItem>& panel_items = model.farming_panel_open ? model.farming_items : (model.active_tool == UiTool::decoration ? model.decor_items : model.build_items);
    ...
    } else { build_scroll_offset_ = 0.0F; build_scroll_max_ = 0.0F; }
...
void GameplayUi::render(SDL_Renderer* renderer) const {
    ...
    if (build_panel_bounds_) {
        ...
        if (model_.farming_panel_open) {
            ...
            draw_text_fit(renderer, panel.x + 12.0F, panel.y + 42.0F, panel.width - 24.0F, model_.farming_stock + " | " + model_.farming_tile_status, 151, 178, 192);
        } else draw_text(renderer, panel.x + 12.0F, panel.y + 27.0F, std::to_string(model_.build_items.size()) + " ITENS", 151, 178, 192);
        SDL_SetRenderDrawColor(renderer, 55, 91, 111, SDL_ALPHA_OPAQUE);
        ...
        if (build_scroll_max_ > 0.0F) {
            ...
            SDL_SetRenderDrawColor(renderer, 35, 68, 85, SDL_ALPHA_OPAQUE); SDL_RenderFillRect(renderer, &track);
            SDL_SetRenderDrawColor(renderer, 113, 163, 185, SDL_ALPHA_OPAQUE); SDL_RenderFillRect(renderer, &thumb);
        }
    }
