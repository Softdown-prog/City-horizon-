...
This module intentionally owns orchestration only. It does not duplicate scene,
render, material or post-processing logic from the canonical City Horizon tools.
"""
...
def _repo_path(value: str, *, must_exist: bool = True) -> Path:
    ...
    try:
        ...
        raise WorkerError("JOB_INVALID", "Job paths must remain inside the repository workspace", {"path": str(path)}) from exc
...
def blender_identity(blender_exe: Path) -> dict[str, str]:
    try:
        ...
        raise WorkerError("BLENDER_NOT_FOUND", "Failed to execute Blender", {"path": str(blender_exe), "error": str(exc)}) from exc
...
def _run(command: list[str], *, error_code: str, env: dict[str, str] | None = None) -> None:
    ...
    if completed.returncode != 0:
        raise WorkerError(error_code, "Command failed", {"returnCode": completed.returncode, "command": command})
...
def _validate_job(job: dict[str, Any]) -> None:
    ...
    if job.get("operation") not in {"canonical_bake", "blender_script"}:
...
def _canonical_bake(job: dict[str, Any], blender_exe: Path) -> tuple[Path, list[dict[str, str]]]:
    asset_config = _repo_path(str(job.get("assetConfig", "")))
    studio_preset = _repo_path(str(job.get("studioPreset") or DEFAULT_STUDIO.relative_to(REPO_ROOT)))
...
def _blender_script(job: dict[str, Any], blender_exe: Path) -> tuple[Path, list[dict[str, str]]]:
    ...
    if missing:
        raise WorkerError("EXPECTED_OUTPUT_MISSING", "One or more declared outputs were not produced", {"missing": missing})

    return output_root, [{"path": script.relative_to(REPO_ROOT).as_posix(), "sha256": _sha256(script)}]
...
def run_job(job_path: Path, blender_exe: Path) -> dict[str, Any]:
    ...
    if job["operation"] == "canonical_bake":
        output_root, inputs = _canonical_bake(job, blender_exe)
    else:
        output_root, inputs = _blender_script(job, blender_exe)
    ...
    return {
        ...
        "job": {"path": job_path.relative_to(REPO_ROOT).as_posix(), "sha256": _sha256(job_path)},
        "inputs": inputs,
