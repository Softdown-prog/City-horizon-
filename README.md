...
- Pendentes de revisão jogável: transições e materiais adicionais de solo; alinhamento visual das fachadas com a rua; embarque e parada da roda gigante; overlay da sorveteria. Rotação de construções (`Z/X` ou `ESQ/DIR`) e preços da padaria já existem; validar no modo de posicionamento/seleção.
