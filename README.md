...
- Terreno: a aba `TERRENO` mantém `LOTES` para comprar parcelas e oferece `GRAMA`/`AREIA` para pintar solo vazio próprio por clique. O save v11 grava as alterações; saves antigos continuam carregando. O cenário inicial referencia texturas de `coast_adjusted/` ausentes no GitHub: a renderização usa `sand_isometric_01.png` nas células de areia e conserva o preenchimento semântico de água. São tiles básicos, ainda sem transições visuais aprovadas entre todos os biomas.
- A roda gigante segura o primeiro frame quando não há atividade. `F11`, com ela selecionada, alterna uma atividade de teste e permite conferir a rotação; `F8` fica reservado à troca do visual do pedestre. Ainda não há embarque de visitantes nem gatilho automático para iniciar a atração. A sorveteria mantém o teste de overlay no mesmo `F11`.
- Pendentes de revisão jogável: transições e materiais adicionais de solo; alinhamento visual das fachadas com a rua; embarque de visitantes na roda gigante; overlay da sorveteria. Rotação de construções (`Z/X` ou `ESQ/DIR`) e preços da padaria já existem; validar no modo de posicionamento/seleção.
