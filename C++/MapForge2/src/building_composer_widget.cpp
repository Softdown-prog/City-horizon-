...
BuildingComposerWidget::BuildingComposerWidget(QWidget* parent)
    ...
    auto* intro = new QLabel(
        ...
        "Typology presets remain editable. Flexible footprints and deterministic controlled variants preserve placement compatibility.", this);
    ...
    form->addRow("Composer contracts", new QLabel(QStringLiteral("architectural_modules_2 / flexible_footprint_1 / controlled_procedural_variation_1"), this));
    ...
    floor_count_spin_ = new QSpinBox(this); floor_count_spin_->setRange(1, 8); floor_count_spin_->setValue(1); form->addRow("Floors", floor_count_spin_);
    floor_height_spin_ = new QSpinBox(this); floor_height_spin_->setRange(36, 132); floor_height_spin_->setValue(82); floor_height_spin_->setSuffix(" px"); form->addRow("Floor height", floor_height_spin_);
    floor_bands_check_ = new QCheckBox("Show floor bands", this); floor_bands_check_->setChecked(true); form->addRow("Floor articulation", floor_bands_check_);

    roof_combo_ = new QComboBox(this); roof_combo_->addItems({"Gable", "Hip", "Pyramid", "Flat", "Shed", "Mansard"}); form->addRow("Roof profile", roof_combo_);
    ...
    auto* variation_form = new QFormLayout();
    procedural_variation_check_ = new QCheckBox("Preview/export deterministic sibling", this); variation_form->addRow("Controlled variation", procedural_variation_check_);
    procedural_variation_seed_spin_ = new QSpinBox(this); procedural_variation_seed_spin_->setRange(0, 999999); procedural_variation_seed_spin_->setValue(101); variation_form->addRow("Variation seed", procedural_variation_seed_spin_);
    procedural_variation_strength_slider_ = new QSlider(Qt::Horizontal, this); procedural_variation_strength_slider_->setRange(0, 100); procedural_variation_strength_slider_->setValue(35); variation_form->addRow("Variation strength", procedural_variation_strength_slider_);
    procedural_vary_palette_check_ = new QCheckBox("Palette", this); procedural_vary_palette_check_->setChecked(true);
    procedural_vary_materials_check_ = new QCheckBox("Materials", this); procedural_vary_materials_check_->setChecked(true);
    procedural_vary_roof_check_ = new QCheckBox("Roof", this); procedural_vary_roof_check_->setChecked(true);
    procedural_vary_modules_check_ = new QCheckBox("Facade modules", this); procedural_vary_modules_check_->setChecked(true);
    auto* variation_flags = new QWidget(this); auto* variation_flags_layout = new QHBoxLayout(variation_flags); variation_flags_layout->setContentsMargins(0, 0, 0, 0);
    for (QCheckBox* check : {procedural_vary_palette_check_, procedural_vary_materials_check_, procedural_vary_roof_check_, procedural_vary_modules_check_}) variation_flags_layout->addWidget(check);
    variation_flags_layout->addStretch(1); variation_form->addRow("Allowed changes", variation_flags);
    auto* next_variant_button = new QPushButton("Next deterministic seed", this); variation_form->addRow("Variant", next_variant_button);
    root->addLayout(variation_form);

    auto* facade_form = new QFormLayout();
    facade_editor_check_ = new QCheckBox("Use manual facade modules", this); facade_form->addRow("Facade editor", facade_editor_check_);
    ...
    facade_module_combo_ = new QComboBox(this); facade_module_combo_->addItems({"Window", "Door", "Storefront", "Sign", "Awning", "Double door", "Garage door", "Balcony", "Marquee", "Air conditioner", "Planter"}); facade_form->addRow("Module", facade_module_combo_);
    ...
    auto* add_module = new QPushButton("Add module", this); auto* remove_module = new QPushButton("Remove selected", this); auto* clear_facade = new QPushButton("Clear selected facade", this);
    facade_buttons->addWidget(add_module); facade_buttons->addWidget(remove_module); facade_buttons->addWidget(clear_facade); facade_buttons->addStretch(1); root->addLayout(facade_buttons);
    ...
    auto* buttons = new QHBoxLayout(); auto* export_button = new QPushButton("Auto Export Validated Package…", this);
    ...
    connect(typology_combo_, qOverload<int>(&QComboBox::currentIndexChanged), this, [this](int index){ applyBuildingTypologyPreset(index); refreshSpecFromControls(); refreshFacadeModuleList(); refreshPreview(); });
    ...
    for (QSpinBox* spin : {footprint_width_spin_, footprint_depth_spin_, footprint_cutout_width_spin_, footprint_cutout_depth_spin_, footprint_annex_depth_spin_, footprint_annex_offset_spin_, footprint_setback_front_spin_, footprint_setback_back_spin_, footprint_setback_left_spin_, footprint_setback_right_spin_})
    ...
    connect(procedural_variation_check_, &QCheckBox::toggled, this, [changed](bool){ changed(); });
    connect(procedural_variation_seed_spin_, qOverload<int>(&QSpinBox::valueChanged), this, [changed](int){ changed(); });
    connect(procedural_variation_strength_slider_, &QSlider::valueChanged, this, [changed](int){ changed(); });
    for (QCheckBox* check : {procedural_vary_palette_check_, procedural_vary_materials_check_, procedural_vary_roof_check_, procedural_vary_modules_check_})
        connect(check, &QCheckBox::toggled, this, [changed](bool){ changed(); });
    connect(next_variant_button, &QPushButton::clicked, this, [this](){ procedural_variation_check_->setChecked(true); procedural_variation_seed_spin_->setValue(procedural_variation_seed_spin_->value() + 1); });
...
void BuildingComposerWidget::applyBuildingTypologyPreset(const int index) {
    if (index <= 0) return;
    const QSignalBlocker b1(footprint_combo_), b2(floor_count_spin_), b3(floor_height_spin_), b4(floor_bands_check_), b5(roof_combo_), b6(roof_pitch_slider_), b7(roof_overhang_slider_), b8(palette_combo_), b9(detail_preset_combo_), b10(wall_material_combo_), b11(roof_material_combo_), b12(material_scale_combo_), b13(material_strength_slider_), b14(material_variation_slider_), b15(material_contrast_slider_), b16(facade_editor_check_), b17(windows_check_), b18(door_check_), b19(awning_check_), b20(sign_check_), b21(chimney_check_), b22(roof_height_slider_), b23(footprint_shape_combo_);

    auto module = [this](BuildingFacadeModuleKind kind, BuildingStreetEdge edge, int floor, float position, float width) { spec_.facade_modules.push_back({kind, edge, floor, position, width, true}); };
    auto windows = [&module](BuildingStreetEdge edge, int floor) { module(BuildingFacadeModuleKind::Window, edge, floor, 0.25F, 0.18F); module(BuildingFacadeModuleKind::Window, edge, floor, 0.75F, 0.18F); };
    auto base = [this](int footprint, int floors, int floor_h, int roof, int pitch, int palette, int detail, int wall_material, int roof_material) {
        footprint_combo_->setCurrentIndex(footprint); footprint_shape_combo_->setCurrentIndex(0); floor_count_spin_->setValue(floors); floor_height_spin_->setValue(floor_h); floor_bands_check_->setChecked(floors > 1);
        roof_combo_->setCurrentIndex(roof); roof_pitch_slider_->setValue(pitch); roof_overhang_slider_->setValue(10); palette_combo_->setCurrentIndex(palette); detail_preset_combo_->setCurrentIndex(detail);
        wall_material_combo_->setCurrentIndex(wall_material); roof_material_combo_->setCurrentIndex(roof_material); material_scale_combo_->setCurrentIndex(1); material_strength_slider_->setValue(45); material_variation_slider_->setValue(35); material_contrast_slider_->setValue(45);
        facade_editor_check_->setChecked(true); windows_check_->setChecked(false); door_check_->setChecked(false); awning_check_->setChecked(false); sign_check_->setChecked(false); chimney_check_->setChecked(false); facade_floor_spin_->setMaximum(floors); spec_.facade_modules.clear();
    };

    switch (index) {
        case 1:
            base(0, 1, 78, 0, 38, 0, 0, 0, 0); chimney_check_->setChecked(true);
            module(BuildingFacadeModuleKind::Door, BuildingStreetEdge::South, 0, 0.50F, 0.20F); module(BuildingFacadeModuleKind::Window, BuildingStreetEdge::South, 0, 0.20F, 0.18F); module(BuildingFacadeModuleKind::Window, BuildingStreetEdge::South, 0, 0.80F, 0.18F); module(BuildingFacadeModuleKind::Planter, BuildingStreetEdge::South, 0, 0.20F, 0.20F); break;
        case 2:
            base(1, 2, 72, 1, 32, 0, 0, 0, 0); chimney_check_->setChecked(true);
            module(BuildingFacadeModuleKind::Door, BuildingStreetEdge::South, 0, 0.20F, 0.16F); module(BuildingFacadeModuleKind::GarageDoor, BuildingStreetEdge::South, 0, 0.70F, 0.36F); windows(BuildingStreetEdge::South, 1); module(BuildingFacadeModuleKind::Balcony, BuildingStreetEdge::South, 1, 0.50F, 0.34F); windows(BuildingStreetEdge::East, 1); break;
        case 3:
            base(1, 1, 78, 3, 18, 1, 1, 2, 1); roof_height_slider_->setValue(10);
            module(BuildingFacadeModuleKind::Storefront, BuildingStreetEdge::South, 0, 0.32F, 0.42F); module(BuildingFacadeModuleKind::DoubleDoor, BuildingStreetEdge::South, 0, 0.76F, 0.20F); module(BuildingFacadeModuleKind::Marquee, BuildingStreetEdge::South, 0, 0.50F, 0.82F); module(BuildingFacadeModuleKind::Sign, BuildingStreetEdge::South, 0, 0.50F, 0.44F); module(BuildingFacadeModuleKind::Planter, BuildingStreetEdge::South, 0, 0.10F, 0.16F); module(BuildingFacadeModuleKind::Planter, BuildingStreetEdge::South, 0, 0.90F, 0.16F); break;
        case 4:
            base(2, 2, 68, 3, 18, 0, 1, 1, 1); roof_height_slider_->setValue(10);
            module(BuildingFacadeModuleKind::Storefront, BuildingStreetEdge::South, 0, 0.38F, 0.48F); module(BuildingFacadeModuleKind::DoubleDoor, BuildingStreetEdge::South, 0, 0.78F, 0.18F); module(BuildingFacadeModuleKind::Sign, BuildingStreetEdge::South, 0, 0.48F, 0.40F); module(BuildingFacadeModuleKind::Storefront, BuildingStreetEdge::East, 0, 0.50F, 0.56F); windows(BuildingStreetEdge::South, 1); windows(BuildingStreetEdge::East, 1); break;
        case 5:
            base(3, 1, 88, 3, 15, 1, 1, 2, 1); roof_height_slider_->setValue(10);
            module(BuildingFacadeModuleKind::Storefront, BuildingStreetEdge::South, 0, 0.28F, 0.42F); module(BuildingFacadeModuleKind::DoubleDoor, BuildingStreetEdge::South, 0, 0.62F, 0.18F); module(BuildingFacadeModuleKind::Storefront, BuildingStreetEdge::South, 0, 0.84F, 0.20F); module(BuildingFacadeModuleKind::Marquee, BuildingStreetEdge::South, 0, 0.52F, 0.86F); module(BuildingFacadeModuleKind::Sign, BuildingStreetEdge::South, 0, 0.50F, 0.36F); module(BuildingFacadeModuleKind::Hvac, BuildingStreetEdge::East, 0, 0.72F, 0.20F); break;
        case 6:
            base(3, 1, 104, 4, 24, 1, 2, 5, 1); module(BuildingFacadeModuleKind::GarageDoor, BuildingStreetEdge::South, 0, 0.46F, 0.56F); module(BuildingFacadeModuleKind::Door, BuildingStreetEdge::South, 0, 0.87F, 0.12F); module(BuildingFacadeModuleKind::Sign, BuildingStreetEdge::South, 0, 0.50F, 0.30F); module(BuildingFacadeModuleKind::Hvac, BuildingStreetEdge::East, 0, 0.60F, 0.22F); break;
        case 7:
            base(3, 2, 76, 1, 34, 0, 0, 4, 0); module(BuildingFacadeModuleKind::DoubleDoor, BuildingStreetEdge::South, 0, 0.50F, 0.20F); windows(BuildingStreetEdge::South, 0); windows(BuildingStreetEdge::South, 1); windows(BuildingStreetEdge::East, 0); windows(BuildingStreetEdge::East, 1); module(BuildingFacadeModuleKind::Marquee, BuildingStreetEdge::South, 0, 0.50F, 0.34F); module(BuildingFacadeModuleKind::Planter, BuildingStreetEdge::South, 0, 0.18F, 0.18F); module(BuildingFacadeModuleKind::Planter, BuildingStreetEdge::South, 0, 0.82F, 0.18F); break;
        case 8:
            base(3, 2, 72, 0, 26, 0, 0, 1, 0); module(BuildingFacadeModuleKind::DoubleDoor, BuildingStreetEdge::South, 0, 0.50F, 0.22F); module(BuildingFacadeModuleKind::Marquee, BuildingStreetEdge::South, 0, 0.50F, 0.34F); for (float p : {0.14F, 0.32F, 0.68F, 0.86F}) module(BuildingFacadeModuleKind::Window, BuildingStreetEdge::South, 0, p, 0.12F); for (float p : {0.14F, 0.32F, 0.50F, 0.68F, 0.86F}) module(BuildingFacadeModuleKind::Window, BuildingStreetEdge::South, 1, p, 0.12F); windows(BuildingStreetEdge::East, 0); windows(BuildingStreetEdge::East, 1); break;
        case 9:
            base(2, 4, 62, 3, 15, 0, 0, 0, 2); roof_height_slider_->setValue(10); module(BuildingFacadeModuleKind::DoubleDoor, BuildingStreetEdge::South, 0, 0.50F, 0.18F); windows(BuildingStreetEdge::South, 0); for (int floor = 1; floor < 4; ++floor) { windows(BuildingStreetEdge::South, floor); module(BuildingFacadeModuleKind::Balcony, BuildingStreetEdge::South, floor, 0.50F, 0.34F); windows(BuildingStreetEdge::East, floor); } break;
        case 10:
            base(2, 4, 64, 3, 15, 1, 1, 2, 1); roof_height_slider_->setValue(10); module(BuildingFacadeModuleKind::Storefront, BuildingStreetEdge::South, 0, 0.32F, 0.38F); module(BuildingFacadeModuleKind::DoubleDoor, BuildingStreetEdge::South, 0, 0.72F, 0.20F); module(BuildingFacadeModuleKind::Marquee, BuildingStreetEdge::South, 0, 0.52F, 0.72F); for (int floor = 1; floor < 4; ++floor) { windows(BuildingStreetEdge::South, floor); windows(BuildingStreetEdge::East, floor); } module(BuildingFacadeModuleKind::Hvac, BuildingStreetEdge::East, 3, 0.82F, 0.16F); break;
        default: break;
    }
}
...
void BuildingComposerWidget::applyDetailPreset(const int index) {
    ...
    const QSignalBlocker a(door_check_), b(windows_check_), c(awning_check_), d(sign_check_), e(chimney_check_), f(door_position_combo_), g(window_pattern_combo_), h(wall_material_combo_), i(roof_material_combo_);
    if (index == 1) {
        door_check_->setChecked(true); windows_check_->setChecked(true); awning_check_->setChecked(true); sign_check_->setChecked(true); chimney_check_->setChecked(false); door_position_combo_->setCurrentIndex(0); window_pattern_combo_->setCurrentIndex(2); wall_material_combo_->setCurrentIndex(2); roof_material_combo_->setCurrentIndex(1);
    } else if (index == 2) {
        door_check_->setChecked(true); windows_check_->setChecked(true); awning_check_->setChecked(false); sign_check_->setChecked(false); chimney_check_->setChecked(false); door_position_combo_->setCurrentIndex(2); window_pattern_combo_->setCurrentIndex(0); wall_material_combo_->setCurrentIndex(3); roof_material_combo_->setCurrentIndex(1);
    } else {
        door_check_->setChecked(true); windows_check_->setChecked(true); awning_check_->setChecked(false); sign_check_->setChecked(false); chimney_check_->setChecked(true); door_position_combo_->setCurrentIndex(1); window_pattern_combo_->setCurrentIndex(1); wall_material_combo_->setCurrentIndex(0); roof_material_combo_->setCurrentIndex(0);
    }
}
...
void BuildingComposerWidget::refreshSpecFromControls() {
    ...
    spec_.floor_count = floor_count_spin_ ? floor_count_spin_->value() : 1; spec_.floor_height_px = floor_height_spin_ ? floor_height_spin_->value() : 82; spec_.wall_height_px = spec_.floor_height_px;
    spec_.floor_bands_enabled = !floor_bands_check_ || floor_bands_check_->isChecked(); spec_.facade_editor_enabled = facade_editor_check_ && facade_editor_check_->isChecked();

    switch (roof_combo_ ? roof_combo_->currentIndex() : 0) {
        case 1: spec_.roof_style = BuildingRoofStyle::Hip; break; case 2: spec_.roof_style = BuildingRoofStyle::Pyramid; break; case 3: spec_.roof_style = BuildingRoofStyle::Flat; break; case 4: spec_.roof_style = BuildingRoofStyle::Shed; break; case 5: spec_.roof_style = BuildingRoofStyle::Mansard; break; default: spec_.roof_style = BuildingRoofStyle::Gable; break;
    }
    spec_.roof_height_px = roof_height_slider_ ? roof_height_slider_->value() : 34; spec_.roof_pitch_degrees = roof_pitch_slider_ ? static_cast<float>(roof_pitch_slider_->value()) : 35.0F; spec_.roof_overhang = roof_overhang_slider_ ? roof_overhang_slider_->value() / 100.0F : 0.10F;
    spec_.roof_fascia_thickness_px = roof_fascia_slider_ ? roof_fascia_slider_->value() / 10.0F : 2.8F; spec_.roof_ridge_scale = roof_ridge_slider_ ? roof_ridge_slider_->value() / 100.0F : 1.0F; spec_.roof_fascia_enabled = !roof_fascia_check_ || roof_fascia_check_->isChecked(); spec_.roof_ridge_enabled = !roof_ridge_check_ || roof_ridge_check_->isChecked(); if (spec_.roof_style == BuildingRoofStyle::Flat) spec_.roof_height_px = 10;

    const int door_position = door_position_combo_ ? door_position_combo_->currentIndex() : 1; spec_.door_position = door_position == 0 ? BuildingDoorPosition::Left : door_position == 2 ? BuildingDoorPosition::Right : BuildingDoorPosition::Center;
    const int window_pattern = window_pattern_combo_ ? window_pattern_combo_->currentIndex() : 1; spec_.window_pattern = window_pattern == 0 ? BuildingWindowPattern::Single : window_pattern == 2 ? BuildingWindowPattern::Strip : BuildingWindowPattern::Pair;

    const int wall_material = wall_material_combo_ ? wall_material_combo_->currentIndex() : 0;
    switch (wall_material) { case 1: spec_.wall_material = BuildingWallMaterial::Brick; break; case 2: spec_.wall_material = BuildingWallMaterial::Concrete; break; case 3: spec_.wall_material = BuildingWallMaterial::Timber; break; case 4: spec_.wall_material = BuildingWallMaterial::Stone; break; case 5: spec_.wall_material = BuildingWallMaterial::MetalPanel; break; case 6: spec_.wall_material = BuildingWallMaterial::Glass; break; case 7: spec_.wall_material = BuildingWallMaterial::Solid; break; default: spec_.wall_material = BuildingWallMaterial::Plaster; break; }
    const int roof_material = roof_material_combo_ ? roof_material_combo_->currentIndex() : 0;
    switch (roof_material) { case 1: spec_.roof_material = BuildingRoofMaterial::MetalSeam; break; case 2: spec_.roof_material = BuildingRoofMaterial::AsphaltShingle; break; case 3: spec_.roof_material = BuildingRoofMaterial::Solid; break; default: spec_.roof_material = BuildingRoofMaterial::CeramicTile; break; }
    const int texture_scale = material_scale_combo_ ? material_scale_combo_->currentIndex() : 1; spec_.material_scale = texture_scale == 0 ? 0.72F : texture_scale == 2 ? 1.45F : 1.0F;
    spec_.material_strength = material_strength_slider_ ? material_strength_slider_->value() / 100.0F : 0.45F; spec_.material_variation = material_variation_slider_ ? material_variation_slider_->value() / 100.0F : 0.35F; spec_.material_contrast = material_contrast_slider_ ? material_contrast_slider_->value() / 100.0F : 0.45F; spec_.material_seed = material_seed_spin_ ? material_seed_spin_->value() : 17;
    spec_.windows = !windows_check_ || windows_check_->isChecked(); spec_.south_door = !door_check_ || door_check_->isChecked(); spec_.south_awning = awning_check_ && awning_check_->isChecked(); spec_.south_sign = sign_check_ && sign_check_->isChecked(); spec_.roof_chimney = chimney_check_ && chimney_check_->isChecked(); spec_.cast_shadow = !shadow_check_ || shadow_check_->isChecked();
    applyPalette(spec_, palette_combo_ ? palette_combo_->currentIndex() : 0);

    spec_.procedural_variation_enabled = procedural_variation_check_ && procedural_variation_check_->isChecked();
    spec_.procedural_variation_applied = false;
    spec_.procedural_variation_seed = procedural_variation_seed_spin_ ? procedural_variation_seed_spin_->value() : 101;
    spec_.procedural_variation_strength = procedural_variation_strength_slider_ ? procedural_variation_strength_slider_->value() / 100.0F : 0.35F;
    spec_.procedural_vary_palette = !procedural_vary_palette_check_ || procedural_vary_palette_check_->isChecked();
    spec_.procedural_vary_materials = !procedural_vary_materials_check_ || procedural_vary_materials_check_->isChecked();
    spec_.procedural_vary_roof = !procedural_vary_roof_check_ || procedural_vary_roof_check_->isChecked();
    spec_.procedural_vary_modules = !procedural_vary_modules_check_ || procedural_vary_modules_check_->isChecked();
}

void BuildingComposerWidget::addFacadeModule() {
    refreshSpecFromControls(); BuildingFacadeModulePlacement module; module.edge = edgeFromIndex(facade_edge_combo_ ? facade_edge_combo_->currentIndex() : 0); module.kind = moduleKindFromIndex(facade_module_combo_ ? facade_module_combo_->currentIndex() : 0); module.floor_index = std::clamp((facade_floor_spin_ ? facade_floor_spin_->value() : 1) - 1, 0, spec_.floor_count - 1); module.position = facade_position_slider_ ? facade_position_slider_->value() / 100.0F : 0.50F; module.width = facade_width_slider_ ? facade_width_slider_->value() / 100.0F : 0.22F; spec_.facade_modules.push_back(module); if (facade_editor_check_) facade_editor_check_->setChecked(true); spec_.facade_editor_enabled = true; refreshFacadeModuleList(); refreshPreview();
}

void BuildingComposerWidget::removeSelectedFacadeModule() {
    if (!facade_module_list_) return; const int row = facade_module_list_->currentRow(); if (row < 0 || row >= static_cast<int>(spec_.facade_modules.size())) return; spec_.facade_modules.erase(spec_.facade_modules.begin() + row); refreshFacadeModuleList(); refreshPreview();
}

void BuildingComposerWidget::clearSelectedFacade() {
    const BuildingStreetEdge edge = edgeFromIndex(facade_edge_combo_ ? facade_edge_combo_->currentIndex() : 0); spec_.facade_modules.erase(std::remove_if(spec_.facade_modules.begin(), spec_.facade_modules.end(), [edge](const BuildingFacadeModulePlacement& module){ return module.edge == edge; }), spec_.facade_modules.end()); refreshFacadeModuleList(); refreshPreview();
}

void BuildingComposerWidget::refreshFacadeModuleList() {
    if (!facade_module_list_) return; facade_module_list_->clear();
    for (const auto& module : spec_.facade_modules) facade_module_list_->addItem(QStringLiteral("%1 / F%2 / %3 / pos %4% / width %5%").arg(BuildingComposer::streetEdgeName(module.edge).toUpper()).arg(module.floor_index + 1).arg(BuildingComposer::facadeModuleKindName(module.kind)).arg(static_cast<int>(module.position * 100.0F)).arg(static_cast<int>(module.width * 100.0F)));
}

void BuildingComposerWidget::refreshPreview() {
    if (!preview_ || !summary_) return;
    BuildingComposerSpec display_spec = spec_;
    if (spec_.procedural_variation_enabled) display_spec = BuildingProceduralVariation::generate(spec_);
    const bool context_enabled = context_preview_check_ && context_preview_check_->isChecked();
    const QImage image = context_enabled ? renderEnvironmentReviewSheet(display_spec, QSize(300, 250)) : BuildingFacadeRenderer::renderReviewSheet(display_spec, QSize(300, 250));
    preview_->setPixmap(QPixmap::fromImage(image).scaled(qMax(240, preview_->width() - 12), qMax(190, preview_->height() - 12), Qt::KeepAspectRatio, Qt::SmoothTransformation));
    const QSize export_frame = BuildingExportPipeline::recommendedFrame(display_spec); const BuildingFootprintMask footprint = BuildingFootprintModel::mask(display_spec); QString footprint_reason; const bool footprint_valid = BuildingFootprintModel::isValid(display_spec, &footprint_reason);
    const QString variant_text = spec_.procedural_variation_enabled ? QStringLiteral("variant seed %1 @ %2%").arg(spec_.procedural_variation_seed).arg(static_cast<int>(spec_.procedural_variation_strength * 100.0F)) : QStringLiteral("base authored building");
    summary_->setText(QStringLiteral("%1 / %2 — %3. Footprint %4 %5×%6 (%7 occupied cells, %8). %9 floor(s) × %10px. %11 roof. Facade: %12 (%13 modules). Preview: %14. Auto-export frame: %15×%16.")
        .arg(BuildingComposer::visualPresetName(display_spec.visual_preset)).arg(BuildingComposer::typologyName(display_spec.building_typology)).arg(variant_text)
        .arg(BuildingFootprintModel::shapeName(display_spec.footprint_shape)).arg(display_spec.footprint_width_tiles).arg(display_spec.footprint_depth_tiles).arg(footprint.occupiedCount()).arg(footprint_valid ? QStringLiteral("valid") : footprint_reason)
        .arg(display_spec.floor_count).arg(display_spec.floor_height_px).arg(BuildingRoofEditorRenderer::roofProfileName(display_spec.roof_style))
        .arg(display_spec.facade_editor_enabled ? QStringLiteral("manual") : QStringLiteral("legacy automatic")).arg(display_spec.facade_modules.size())
        .arg(context_enabled ? QStringLiteral("terrain + sidewalk + road + footprint mask") : QStringLiteral("asset only")).arg(export_frame.width()).arg(export_frame.height()));
}

void BuildingComposerWidget::exportAsset() {
    const QString output_dir = QFileDialog::getExistingDirectory(this, "Auto Export Validated Building Package", QString()); if (output_dir.isEmpty()) return;
    BuildingComposerSpec export_spec = spec_; if (spec_.procedural_variation_enabled) export_spec = BuildingProceduralVariation::generate(spec_);
    QString stem, error; BuildingExportValidation validation; const bool exported = BuildingExportPipeline::exportPackage(export_spec, output_dir, &stem, &validation, &error); const QString validation_path = QDir(output_dir).filePath(stem + QStringLiteral("_validation.json"));
    if (!exported) { summary_->setText(QStringLiteral("EXPORT BLOCKED: %1. Validation report: %2").arg(error, validation_path)); return; }
    summary_->setText(QStringLiteral("AUTO EXPORTED: %1/%2_* — %3. Flexible footprint, typology, facade/floor and deterministic variation metadata recorded in manifest.").arg(output_dir, stem, validation.summary()));
}
